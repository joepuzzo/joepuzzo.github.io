...
class Resume extends Component {
  ...
  async printDocument() {
    ...
    const inputs = document.getElementsByClassName('resume-section');

    const promises = Array.prototype.map.call( inputs, ( input ) => html2canvas(input, { scale: 0.82 }) );
    ...
    images.forEach( ( image, i ) => {
      pdf.addImage(image, 'JPEG', 2, 10);
      if ( i < images.length - 1) {
        pdf.addPage();
      }
    });

    pdf.save("JoePuzzoResume.pdf");
  }

  render() {
    return (
      <div>
        <div className="bg-offwhite section">
        ...
        <div style={{ textAlign: 'center' }} className="mt-2 mb-2">
          <button className="button button-primary" onClick={() => { this.printDocument(); }}>Download</button>
        </div>
      </div>
    )
  }
